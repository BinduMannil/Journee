...
| Smart Day Planner | ✅ | `planSmartDay(destinationId)` orders a destination's attractions through the day by `bestTimeOfDay`, busiest-first within a slot (beat the crowds); pure, over the attractions seed; carries `ATTRACTIONS_DATA_NOTE` | Multi-day packing + travel time between sites; tie to live hours; UI |
...
| Nightlife & Bars | ✅ (seed) | After-dark venues/areas per destination (bar/cocktail/club/live-music/rooftop/pub/cultural-evening/night-market) with vibe + typical hours; `getNightlife` / `nightlifeByKind` / `nightlifeByVibe`; `NIGHTLIFE_DATA_NOTE` (culturally aware, not a live listings feed) | Live listings, expanded coverage, UI |
| Beaches & Swimming Spots | ✅ (seed) | Beaches/lakes/pools per destination with water type, vibe + swimmable flag; honest `hasCoast` for landlocked cities; `getBeaches` / `swimmableSpots` / `spotsByWaterType`; `BEACHES_DATA_NOTE` (heed local flags, not a live conditions feed) | Live water/safety conditions, more spots, UI |
| Family & Kids Activities | ✅ (seed) | Kid-friendly activities per destination with age-suitability + stroller-friendly flag; `getFamilyActivities` / `activitiesForAge` / `activitiesByKind`; `FAMILY_DATA_NOTE` (suitability depends on the child — verify) | Expanded coverage, UI |
...
| Tourist Sites & Attractions | ✅ (seed) | Top sites per destination with cost band/approx USD, best time of day + why (light/weather/heat/crowds), typical busyness, typical hours, an honest booking note (no fabricated/affiliate URLs), and **experience tags**; `getAttractions` / `attractionsByBestTime` / `freeAttractions` / **`sitesForExperience`** / `experiencesAvailable`; `ATTRACTIONS_DATA_NOTE` | Live hours/ticketing/crowd feeds, real booking integration, more sites, UI |
...
- ✅ **Best intercity options / best cities & sites to club together / best
  sites by chosen experience** (product-owner requests) — shipped (seed
  `intercity`, `itinerary-combos`, and experience tags on `attractions` +
  `sitesForExperience` / `experiencesAvailable`).
